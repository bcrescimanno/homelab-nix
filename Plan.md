...
- [ ] **Presence sensor for the Boys Bathroom** — make the lights auto-off presence based instead of time based. Today `modules/ha-bathroom-lights.nix` turns the lights off after 15 minutes, with a 19:00–20:30 exception added because it cut the lights mid-shower. A timer can't tell a long shower from a forgotten light. Plan (2026-09-13): **wasp-in-a-box** with an IKEA MYGGSPRAY (~$10, Thread PIR) plus an Eve Door & Window. Motion while the door is closed holds "occupied" until the door opens, so a still person in the shower is covered by the door rather than by radar. Door open + no motion ~2 min → off, with a 15-min open-door fallback because kids leave the door open. If that habit makes it pointless, use a Meross MS605 (mmWave) alone. Cats can only keep the light on longer, never turn it on. Replaces the shower window and the 15-minute timer. Detail in `devices/presence-sensors.md` → "use cases and pets".
