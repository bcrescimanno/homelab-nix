...
| Technitium declarative Nix config | **Investigated — not possible today.** Technitium stores config in a binary format; all configuration must go through the HTTP API. nixpkgs has a `services.technitium-dns-server` module but it only handles enable/disable + firewall. A third-party `technitium-configurator` tool exists (YAML → API) but adds complexity with no real advantage over `configure-technitium.sh`. Watch for Technitium issue #1084 (native JSON/YAML config) — if implemented, a proper Nix module becomes viable. |
...
| **Vaultwarden / password manager** | Staying on 1Password for now — the SSH agent integration (signing git commits, SSH auth via Touch ID) is too valuable to give up. Bitwarden/Vaultwarden has no equivalent SSH agent. Revisit if a self-hosted password manager adds a first-class SSH agent feature comparable to 1Password's. |
