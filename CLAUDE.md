...
- `modules/arr-stack.nix` — pirateship media stack containers (gluetun VPN kill switch, qbittorrent, radarr, sonarr, prowlarr, lidarr, sabnzbd) plus the native recyclarr service; `qbittorrent-port-sync` systemd service syncs the gluetun forwarded port to qBittorrent. **Jellyfin is not here** — it runs natively on orthanc (`modules/jellyfin.nix`). **No login prompt on the LAN**: the four Servarr apps run `AuthenticationRequired = DisabledForLocalAddresses`, re-asserted by `arrAuthPreStart` because they rewrite `config.xml` themselves, and qBittorrent has a `WebUI\AuthSubnetWhitelist`. `TrustedNetworks` is **mandatory, not hardening** — since the CVE-2026-30975 fix a patched build treats any request carrying `X-Forwarded-For` from an untrusted peer as non-local, so behind Caddy the setting is inert without it. Tailscale clients still get the login form (100.64.0.0/10 is not RFC1918). Read the header comment before changing any of it
...
- `modules/ha-pizza-preheat.nix` — Discord ping when the wall oven finishes preheating **on the Pizza setting** (HA package `pizza_preheat`, read-only in the UI). Posts to a **channel webhook** via `rest_command`, not through HA's `discord` integration: that one is `config_flow`-only, so its bot token would live in `.storage` and its notify entity would be invisible to Nix. The mention only pings because the member id is repeated under `allowed_mentions` (`parse: []` there also makes an accidental `@everyone` impossible). Home Connect has **no event platform** — its appliance events are enum sensors reading present/confirmed/off — so the trigger is a state trigger to `present` on **both** `sensor.wall_oven_pre_heat_finished` and `sensor.wall_oven_regular_pre_heat_finished` — `pre_heat`, and `wall_oven_` while the program select on the same device is still `oven_`, because entity_ids do not follow a device rename. Only the regular event has ever fired; fast pre-heat stays wired as the unused path. `not_from` excludes `unavailable`/`unknown` for the same cloud flapping as `ha-hood-light.nix`, and a 10-minute `last_triggered` guard catches the restart that `not_from` cannot see. The program is read from `select.oven_active_program` alone: `select.oven_selected_program` has **never** reported a program (Home Connect only publishes it under remote control) and `switch.oven_program_pizzasetting` is a leftover that is always `unavailable`. Measured shape of every run: `active_program` → pizza, 13–17 min, then `present` for ~29s. The webhook URL **and** the member id are sops secrets rendered into `configDir/secrets.yaml` by `sops.templates."ha-secrets.yaml"` — further HA `!secret` values belong in that template. **A missing key there fails the whole HA config load**, not just this automation
...
- `modules/music-assistant.nix` — Music Assistant on rivendell (port 8095, `listen.theshire.io`), reading `/var/lib/media/music` over NFS. **Does not replace Navidrome** — the two run side by side: Music Assistant drives local multi-room playback to WiiM/HomePods, Navidrome serves remote/mobile over OpenSubsonic. The NixOS module uses `DynamicUser`, so the export must allow reads from an ephemeral UID. **The wiim provider needs `pkgs/pywiim.nix`**, which nixpkgs does not package; it is appended to the unit's `PYTHONPATH` and switches itself off once nixpkgs lists it. Its version must track the `pywiim==` pin in MA's wiim `manifest.json`. A provider that fails to import shows in the MA UI as "Reconfigure", with nothing behind the button
...
- `modules/deadman.nix` — the **external** dead-man switch, on mirkwood and orthanc. Every alert path in this lab ends at rivendell's ntfy, so rivendell's own death — or just a broken ntfy — was unreportable. Deliberately **not** a liveness ping: each host publishes a random token to a dedicated `deadman-probe` ntfy topic on rivendell and reads it back through the poll API, and **only a verified round-trip pings healthchecks.io**. A bare ping would stay green while ntfy was dead, which is the actual gap. **One check per host, never a shared one** (either host would keep a shared check green and mask the other). A failed round-trip **exits 0 on purpose** — it has already POSTed to `hc-ping.com/<uuid>/fail`, and the only local notifier is the ntfy just proved broken; the unit exits non-zero *only* when hc-ping.com itself is unreachable, which is the case where the local path still works. healthchecks.io alerts out via a **public ntfy.sh topic**, never `ntfy.theshire.io`, or the single point of failure is just rebuilt one level up. Unprovisioned is not silence: it writes `homelab_deadman_provisioned 0` to the textfile collector and `grafana.nix` alerts on it. Setup that cannot live in Nix (check periods, the ntfy integration) is recorded in the module header
...
- `ha_discord_pizza_webhook` / `ha_discord_pizza_user_id` — the Discord channel webhook URL and the member id to tag, for `modules/ha-pizza-preheat.nix`. **Quote the id** (`"123456789012345678"`): a bare snowflake is a YAML integer and sops-install-secrets takes only strings, failing activation with `the value of key '' is not a string`. Rendered together into `/var/lib/homeassistant/config/secrets.yaml` by `sops.templates."ha-secrets.yaml"` and read with HA's `!secret`. The id is not itself a credential; it is a secret so the public repo carries no member id. **Both must exist before deploying** — HA fails its whole config load on an unresolvable `!secret`, which takes Home Assistant down rather than just this automation

**mirkwood** (`secrets/mirkwood.yaml`):
- `grafana_env` — `GF_SECURITY_ADMIN_PASSWORD`
- `deadman_ping_url` — this host's own `https://hc-ping.com/<uuid>` heartbeat URL; declared by `deadman.nix`. **A different value per host** (mirkwood and orthanc own separate checks, on purpose). Anything not starting `https://hc-ping.com/` is treated as unprovisioned and alerts rather than failing quietly
