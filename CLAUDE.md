...
- `modules/ha-bathroom-lights.nix` — turns the Boys Bathroom lights off once they have been on for 15 minutes (HA package `boys_bathroom_lights`, read-only in the UI). It has **two triggers on purpose**: a state trigger with `for`, plus a `homeassistant: start` trigger that waits 15 min and re-checks. A `for` countdown lives in memory and is lost on restart, so without the second trigger a light left on across a restart could stay on forever. Durations are YAML mappings (`{ minutes = 15; }`), never `"00:15:00"` strings.
- `modules/ha-hvac-openings.nix` — pauses `climate.main_floor` while the house is open (HA package `hvac_openings`, read-only in the UI). **Any** sensor in `openings` open for 60s → save the mode to `input_text.hvac_openings_saved_mode`, set `off`, push to both phones. **All** sensors reporting `off` → restore immediately, silently. `unavailable` is not closed: a dark sensor never pauses and blocks resume (infra alert after 30 min). A manual change to heat/cool/heat_cool while paused abandons the pause. Only the kitchen door today — **adding window sensors is adding entity IDs to `openings`**; the header holds the expansion plan (per-sensor delays, multi-zone, dead-sensor escape hatch).
...
- `modules/piper.nix` — local Piper TTS over Wyoming on rivendell (`127.0.0.1:10200`, zeroconf off) so HA announcements to the HomePods (via Music Assistant's AirPlay players; `tts.speak` sets `announce: true`) use a local voice instead of Google Translate. **Voice NOT decided — do not merge/deploy as-is**: the default is a placeholder pending more listening tests on a five-voice English-male shortlist recorded in the module header. Piper has no en_AU voice. The HomePods' own Siri voice is unreachable (AirPlay carries audio only; Intercom has no API). After deploy, the Wyoming integration must be added once in the HA UI (no YAML setup exists).
